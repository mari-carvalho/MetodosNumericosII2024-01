...
def calculate_3_rk4():
    ...
    # Método de Ruge-Kutta de 4ª Ordem:
    for i in range(0,n-1): # deve ir até n-1 porque a solução de y[i+1] é resolvida até o tempo de 1s (y[i+1] é 2), sendo que o tempo final é 2
        ...
        T[i+1] = T[i] + (1/6)*(k1_T + (2*k2_T) + (2*k3_T) + k4_T) # solução numérica da temperatura com o método de runge-kutta 4ª ordem 
        C[i+1] = C[i] + (1/6)*(k1_C + (2*k2_C) + (2*k3_C) + k4_C) # solução numérica da composição com o método de runge-kutta 4ª ordem
    print('T', T)
    print('C', C)

    # Plotagem da Solução do Sistema de EDO com o Método de Runge-Kutta de 4ª Ordem:
    ...
    plt.title('Solução Numérica da Temperatura - RK4')
    plt.xlabel('Tempo(s)')
    plt.ylabel('T(t)')
    plt.grid(True)
    plt.legend()
    ...
    plt.title('Solução Numérica da Composição - RK4')
    plt.xlabel('Tempo(s) [°C]')
